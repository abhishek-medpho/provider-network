...
import { respondToGig } from "@/lib/actions/gig-respond";
import { gigTaskLabel } from "@/lib/gigs/labels";
import { RespondActions } from "./_components/RespondActions";

export const metadata: Metadata = {
  title: "Job request — Labstack Provider",
};
...
export default async function RespondPage({
  ...
  async function respond(decision: "INTERESTED" | "DECLINED") {
    "use server";
    return await respondToGig(token, decision);
  }
  ...
  return (
    <main className="max-w-md mx-auto px-4 py-8">
      ...
      <h1 className="text-xl font-semibold text-zinc-900 mb-4">
        A job near you — are you available?
      </h1>

      {/* Lightweight details only — NO patient name/address until selected. */}
      <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-3 mb-6">
        ...
        <dl className="grid grid-cols-[90px_1fr] gap-y-2 text-sm">
          ...
          {resp.distanceKm != null && (
            <>
              ...
              <dd className="text-zinc-900">~{resp.distanceKm} km away</dd>
            </>
          )}
          {gig.payText && (
            <>
        ...
        <p className="text-xs text-zinc-500 border-t border-zinc-100 pt-3">
          Exact address is shared once you&apos;re confirmed for the job.
        </p>
      </div>

      <RespondActions respond={respond} initialStatus={resp.status} />
